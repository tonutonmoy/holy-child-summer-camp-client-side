...
const InstructorPage = () => {
  ...
  useEffect(() => {
    fetch(
      "https://holy-child-summer-camp-server-side.vercel.app/allInstructors"
    )
      .then((res) => res.json())
  ...
  console.log(allInstructors, "hellow");

  return (
    <div className={`py-40   ${toggle ? "bg-[#090909] " : ""} `}>
      <h2
        className={`text-[25px] md:text-[30px] lg:text-[30px] xl:text-[35px]  2xl:text-[40px] font-medium text-center my-20 lg:font-semibold ${
          toggle ? "text-white" : "text-[#2D3747] "
        } rounded-md  `}
      >
        {" "}
        All Instructors{" "}
      ...
      <div className=" grid md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4   w-[90%] mx-auto gap-5">
        {allInstructors?.map((data) => (
          <div
            key={data?._id}
            className="group shadow-md  relative m-0 flex h-72 w-full rounded-xl shadow-gray-900  overflow-hidden z-0  ring-gray-900/5 sm:mx-auto sm:max-w-lg bg-black/90 "
          >
            <div
              className={`z-0 ${
                toggle && "border-[1px]"
              }  h-full w-full overflow-hidden rounded-xl opacity-100 transition duration-300 ease-in-out group-hover:opacity-70 dark:border-gray-700 dark:opacity-70`}
            >
              <img
                src={data?.userPhoto}
